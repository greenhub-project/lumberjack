...
const greenhub = {
  ...
  server: {
    url: {
      status: 'http://greenhub.hmatalonga.com/status.json',
  ...
  export(model, filters, options) {
    ...
    this.makeExport(model, params);
  },

  list(options) {
    if (!this.isLoggedIn()) return;

    axios.get(this.makeUrl('models'), {
  ...
  lumberjack(model, filters, options) {
    ...
    if (options.output && !options.output.endsWith('.json')) {
      console.log(logSymbols.error, chalk.red('filename with wrong extension!'));
      return;
    }
  ...
  status(options) {
    ...
    axios.get(this.makeUrl('status'), {
      timeout: Math.min(options.timeout * 1000, 15000)
    })
      .then(response => {
  ...
  makeExport(model, params) {
    console.log(chalk.blue('Fetching data from the server...'));

    params.export = true;

    axios.get(this.makeUrl(model), {
      params: this.bundleParams(params),
      onDownloadProgress: function(progressEvent) {
        console.log('Downloading file...', progressEvent);
      },
      ...
      .then(response => {
        ...
        fs.writeFile(params.output, response.data, function (err) {
          ...
          console.log(logSymbols.success, chalk.bold(params.output), 'created!');
        });
      })
      .catch(() => {
        console.log(logSymbols.error, chalk.red(this.server.errors.failed));
    ...
    axios.get(this.makeUrl(model), {
      ...
      .then(response => {
        if (params.output) {
          ...
          if (contents === null) {
            console.log(logSymbols.error, 'An error occurred reading the results');
            return;
          }
  ...
  makeUrl(method) {
    ...
    method = '/' + method;
    if (process.env.NODE_ENV === 'development') {
      return this.server.url.development + '/api/v' + this.server.api.version + method;
    }
    return this.config.get('server') + '/api/v' + this.server.api.version + method;
  },
  ...
  buildSearchParams(filters, options) {
    ...
    if (options.everything) {
      ...
      if (list.length === 0) {
        console.log(logSymbols.warning, 'Invalid `with` parameter!', options.with);
        return {};
