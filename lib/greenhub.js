...
const greenhub = {
  ...
  token: function(options) {
    ...
    if (options.newToken) {
      ...
      axios.post(this.makeUrl('/token/new'), this.makeParams({}, ''))
      .then(response => {
  ...
  makeQuery: function(model, params) {
    ...
    .then(response => {
      ...
      inquirer.prompt([
        ...
        if (answers.action === 'finish') return;
  ...
  makeUrl: function(method) {
    return this.server.url.test + '/api/v1' + method;
  },
  ...
  buildSearchParams: function(filters, options) {
    ...
    if (options.date) {
      ...
      if (validate) {
        ...
        console.log(logSymbols.warning, 'Invalid parameter!', options.date);
        return {};
      }
      ...
      if (this.validateDateRange(dates) && options.range !== '..') {
        ...
        console.log(logSymbols.warning, 'Invalid parameter!', options.range);
        return {};
      }
    }
